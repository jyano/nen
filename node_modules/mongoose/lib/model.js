...
// Inherits from Document.  All Model.prototype features are available on  top level (non-sub) documents.


Model.prototype.__proto__ = Document.prototype
Model.prototype.db // md's, conn     @api public   @property db
Model.prototype.collection // md's cl    @api public @property collection
Model.prototype.modelName// md's name   @api public    @property modelName
Model.prototype.$__handleSave = function $__handleSave(options) {
  var self = this;
  var innerPromise = new Promise;
  if (!options.safe && this.schema.options.safe) {options.safe = this.schema.options.safe}
...
Model.schema //md's sc   @property schema  @receiver Model  @api public
Model.db //md's cn  @property db  @receiver Model  @api public
Model.collection;//md's cl    @property collection  @receiver Model  @api public
Model.base //Base Mongoose instance the model uses.  @property base  @receiver Model  @api public
Model.discriminators //Registered discriminators for this model.   @property discriminators    @receiver Model     @api public
...
var excludeIdReg = /\s?-_id\s?/, excludeIdRegGlobal = /\s?-_id\s?/g;
...
function populate(model, docs, options, cb) {
  ...
  if (0 === docs.length || docs.every(utils.isNullOrUndefined)) {return cb()}

  modelsMap = getModelsMapForPopulate(model, docs, options);
  rawIds = getIdsForAndAddIdsInMapPopulate(modelsMap);

  var i, len = modelsMap.length, mod, match, select, promise, vals = [];
