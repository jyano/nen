...
$n.cS = $n.createServer
$n.S=function(){
    var g = G(arguments), o, s
    o = g.F_ ? {fn: g.f} : {}
    if (o.fn) {s = this.cS(function (k) {o.fn($sK(k))})}
    else {s = this.cS()}
    s.l = function (fn) {
        this.listen(fn);
        return this
...
// The connectionListener argument is
// automatically set as a listener for 'cn' ev.

dfOp={

    allowHalfOpen: false,
    //If allowHalfOpen is true,
// then the socket won't automatically send a FIN packet when the other end of the socket sends a FIN packet.
...
//If pauseOnConnect is true,
// then the socket associated with each incoming connection will be paused,
// and no data will be read from its handle. This allows connections
// to be passed between processes without any data being read by the original process.
// To begin reading data from a paused socket, call resume().
}


//   echo server:listens for cns on port 8124:
sv = $net.S(function(cl){ //'connection' listener
    $l('cl cn');
    cl.on('end', function() {$l('cl discn')})
    cl.w('hello\r\n');c.pipe(c);
})
sv.lsn(8124, function() {$l('server bound');}); //'listening' listener
...
//        When the socket is established,   'connect' event will be emitted. Has the same events as 'net.Socket'.
...
// localAddress: Local interface to bind to for network cns
// localPort: Local port to bind to for network cns
...
//        Common options are:
//        allowHalfOpen: if true,
// the socket won't automatically send a
// FIN packet when the other end of the socket
// sends a FIN packet. Defaults to false.
// See 'end' event for more information.
//    The connectListener pam
// will be added as an listener for the 'connect' event.
//        Here is an example of a client
// of echo server as described previously:
//


cl=   client = $net.connect({port: 8124},
        function() { //'connect' listener
            $l('connected to server!');
            cl.w('world!\r\n');
        })

cl.on('data', function(d){$l(d.tS());cl.end()})

cl.on('end',function(){$l('disconnected from server')});
  //  To connect on the socket /tmp/echo.sock the second line would just be changed to
//
   cl = net.connect({path: '/tmp/echo.sock'});

net.connect//(port[, host][, connectListener])#
net.createConnection//(port[, host][, connectListener])#
//Creates a TCP connection to port on host. If host is omitted, 'localhost' will be assumed. The connectListener parameter
// will be added as an listener for the 'connect' event.
...
// the last pam cb will be added as
...
// the address family name and port of the server as reported by the operating system.
// Useful to find which port was assigned when giving getting an OS-assigned address.
// Returns an object with three properties,
// e.g. { port: 12346, family: 'IPv4', address: '127.0.0.1' }

sv = n.S(function(k){k.end("goodbye\n")})


sv.listen(function(){// grab a random port.
    address = server.address();$l("opened server on %j", address)
})
...
//
//    Note that the data will be lost
// if there is no listener
// when a Socket emits a 'data' event.
//
//        Event: 'end'#
//Emitted when the other end of the socket sends a FIN packet.
//        By default (allowHalfOpen == false)
// the socket will destroy its file descriptor
// once it has written out its pending write queue.
// However, by setting allowHalfOpen == true
// the socket will not automatically end()
// its side allowing the user to write arbitrary
// amounts of data, with the caveat that the user
// is required to end() their side now.
//
//        Event: 'timeout'#
...
// The argument had_error is a boolean
// which says if the socket was closed due
